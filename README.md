...
- **Two views** — a grouped **List** and a **Board** (kanban). In the List, drag cards to order them or onto each other to group them, on Running, Closed and Archived alike, or **sort** each category by last update or ticket number.
...
- **Tickets & PRs on the card** — the ticket id and a GitHub icon sit on every card, clickable straight through to your tracker. A session can carry **several** of each (a task split across two PRs, a sub-task the session filed): the icon then shows a count and opens a picker. Editable from the app, and `/save-session` · `/close-session` keep the lists filled as the work grows.
...
| Tests | Rust unit tests (261, `cargo test`) + Jest (277, renderer logic) + Playwright smoke tests (59, the renderer in a real browser) + the Python unittest files of the hooks and skills/lib (86, `npm run test:py`) — 683 total |
