...
### Fixed
- **"End session" now closes with an AI wrap-up** — the embedded terminal's *End session ✕* button used to just kill the process, leaving the session **stale** (nothing written to notes.md). It now injects `/close-session` into the live session so Claude writes the full summary, waits until that close is recorded, then kills the pty → the session lands in **Closed**. Falls back to a plain end (with an "end anyway" prompt) if no wrap-up appears (e.g. the session is in plan mode).
