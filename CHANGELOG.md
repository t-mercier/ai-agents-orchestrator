...
### Added
- **Usage status bar (opt-in)** — a slim bottom bar showing your **model**, the **5-hour** and **weekly** rate-limit windows (colour-coded by threshold), and the current **context %**. Claude Code only hands that data to a `statusLine` command, so a bundled piggyback wrapper (`scripts/statusline-usage-piggyback.sh`) persists it to `~/.claude/statusline-cache.json`; the app reads it **read-only** and renders the bar (hidden when the cache is absent, dimmed when stale). Never touches your config.
